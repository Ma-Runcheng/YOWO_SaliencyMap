...
class YOWO(nn.Module):
    ...
    @torch.no_grad()
    def inference(self, video_clips, cam=False):
        """
        Input:
    ...
    def inference_CAM(self, video_clips, cam=False):
        """
        Input:
            video_clips: (Tensor) -> [B, 3, T, H, W].
        """
        ...
        cls_pred = pred[:, K:K * (1 + C), :, :].permute(0, 2, 3, 1).contiguous().view(B, -1, C)
        reg_pred = pred[:, K * (1 + C):, :, :].permute(0, 2, 3, 1).contiguous().view(B, -1, 4)
        return cls_pred[:, 0, :].squeeze()


    def forward(self, video_clips, targets=None, cam=False):
        """
        Input:
            video_clips: (Tensor) -> [B, 3, T, H, W].
            targets: (List) -> [[x1, y1, x2, y2, label, frame_id], ...]
        """
        video_clips.requires_grad_()
        if cam:
            return self.inference_CAM(video_clips)
        if not self.trainable:
            ...
            # [B, K*C, H, W] -> [B, H, W, K*C] -> [B, M, C], M = HWK
            # 预测阶段
            # 每个bbox有5个参数（位置4+confidece）和类别数（jhmdb21个类别）所以pred.shape=[8, 5*(21+5), 7, 7]=[8, 130, 7, 7]
            # confidence,目标概率预测  [8, 245, 1]
            conf_pred = pred[:, :K, :, :].permute(0, 2, 3, 1)
            # 类别概率预测 [8,245,21]
            cls_pred = pred[:, K:K*(1 + C), :, :].permute(0, 2, 3, 1).contiguous().view(B, -1, C)
            # regression，位置预测 [8,245,4]
            reg_pred = pred[:, K*(1 + C):, :, :].permute(0, 2, 3, 1).contiguous().view(B, -1, 4)
