...
def train():
    ...
    if args.load_path is not None:
        ...
        model.load_state_dict(checkpoint['model'])  # 加载模型的参数
        optimizer.load_state_dict(checkpoint['optimizer'])  # 加载优化器的参数
        checkpoint['args'].load_path = args.load_path
        args = checkpoint['args']
        print("-----------load checkpoint---------------")
        print(args)
        print('have trained epoch: ', start_epoch)
        print('Remaining epoch', max_epoch - start_epoch)
        print("--------------------------------------------------")
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        checkpoint_path = os.path.join('C:/Users/MaRuncheng/Desktop/graduation project/YOWO/PyTorch_YOWO/weights/jhmdb21/yowo', weight_name)
